...
TypeHunter::generateConstraint(xed_decoded_inst_t &xedd)
{
    ...
    // we return directly, 
    if(TrackState -> trackVariable(xedd, typeContainer)){
        return;
    }
    ...
    switch(iclass)
    {
        ...
        case XED_ICLASS_MOV:
        {
            ...
            {
                AbstractVariable* var1 = TrackState -> getMemoryVariable(xedd, typeContainer);
        ...
        case XED_ICLASS_MOVZX:
        {
            ...
            if(IsMemoryOperand(xedd, 1)){
                ...
                var2 = TrackState -> getMemoryVariable(xedd, typeContainer);
        ...
        case XED_ICLASS_ADD:
        {
            ...
            else if(IsMemoryOperand(xedd, 0) && IsRegisterOperand(xedd, 1))
            {
                ...
                var1 = TrackState -> getMemoryVariable(xedd, typeContainer);
                var2 = TrackState -> getRegister(src_reg);
        ...
        case XED_ICLASS_INC:
        {
            ...
            if(IsMemoryOperand(xedd, 0)){
                ...
                var = TrackState -> getMemoryVariable(xedd, typeContainer);
                size = MemoryOperandSize(xedd, 0);
            }
        ...
        case XED_ICLASS_SETZ:   // unknown
        {
            ...
            if(IsRegisterOperand(xedd, 0)){
                ...
                size = MemoryOperandSize(xedd, 0);
                AbstractVariable* var = TrackState -> getMemoryVariable(xedd, typeContainer);
        ...
        case XED_ICLASS_LEA:
        {
            ...
            if(base_reg == XED_REG_ESP || base_reg == XED_REG_EBP)
            {
                ...
                // we can also figure which variable it point to.
                // equal relation.
                AbstractVariable* contain = TrackState -> getMemoryVariable(xedd, typeContainer);
                if(contain != NULL)
                {
                    TrackState -> setTempVariable(dest_reg, calcResult);

                    //Constraint *Q = ConstraintADD((TYPE*)NumType(size), var1, NULL, calcResult);
                    Constraint *Q = ConstraintLEA(contain, calcResult); // generate a pointer type, with the contain as the result.
                    typeContainer.insert(typeContainer.end(), Q);
                }
                else{
                    printf("one of the operand is NULL\n");
            ...
            if(index_reg == XED_REG_INVALID)   // the base register add a displacement.
            {
            ...
            else{    // both base and index register is valid.
        ...
        case XED_ICLASS_IDIV:
        {
            ...
            size = RegisterOperandSize(xedd, 1);
            AbstractVariable* var2 = NULL; //= TrackState -> getMemoryVariable(xedd, typeContainer);
        ...
        case XED_ICLASS_CALL_FAR:
        {
            ...
            calcResult = new AbstractVariable();
            calcResult -> region = Temporary;
            calcResult -> size = 4; // we assume it is 4, this will be changed later.


            // the EAX will hold the return value, so we create a temporary varibale here.
            // if the function doesn't return any value, this is unharm. 
            TrackState -> setTempVariable(XED_REG_EAX, calcResult);
            break;
        }
